  ...
  methods: {
    ...
    fixInconsistencies () {
      this.$buefy.dialog.confirm({
        ...
        type: 'is-warning',
        onConfirm: async () => {
          await ky.post('/api/organize/inconsistencies/fixall', { json: {} })
    ...
    async pollFixStatus () {
      ...
      apply(await ky.get('/api/organize/inconsistencies/status').json())
      if (this.fixPoll) { clearInterval(this.fixPoll); this.fixPoll = null }
      if (this.fixRunning) {
        this.fixPoll = setInterval(async () => {
          const st = await ky.get('/api/organize/inconsistencies/status').json()
          apply(st)
          if (!st.running) {
